...
class MasterTest(SemiAutomaticCleanupTestBase):
    ...
    def _validate_master(self, master, headline=None):
        ...
        self.assertEquals(master.get(const.GNM_ASSET_TITLE), 'A TITLE')
        self.assertEquals(master.get(const.GNM_ASSET_DESCRIPTION), 'DESCRIPTION')
        self.assertEquals(master.get(const.GNM_ASSET_KEYWORDS), None)
        self.assertEquals(master.get(const.GNM_ASSET_FILENAME), 'FILENAME')
        self.assertEquals(master.get(const.GNM_ASSET_FILMING_LOCATION), 'OUTSIDE')
        self.assertEquals(master.get(const.GNM_ASSET_CREATEDBY), str(self.user.pk))
        self.assertEquals(master.get(const.GNM_ASSET_FILE_CREATED), self.datetime_now)
        self.assertEquals(master.get(const.GNM_ASSET_FILE_LAST_MODIFIED), self.datetime_now)
        self.assertEquals(master.get(const.GNM_MASTERS_DAILYMOTION_NOMOBILEACCESS), 'no_mobile_access')
        self.assertEquals(master.get(const.GNM_MASTERS_DAILYMOTION_DESCRIPTION), 'DESCRIPTION')
        self.assertEquals(master.get(const.GNM_MASTERS_DAILYMOTION_PUBLISH), datetime.date.today().strftime(const.VS_DATEFORMAT))
        self.assertEquals(master.get(const.GNM_MASTERS_DAILYMOTION_CONTAINSADULTCONTENT), 'contains_adult_content')
        self.assertEquals(master.get(const.GNM_MASTERS_DAILYMOTION_KEYWORDS), None)
        self.assertEquals(master.get(const.GNM_MASTERS_DAILYMOTION_AUTHOR), 'SOME AUTHOR')
        self.assertEquals(master.get(const.GNM_MASTERS_DAILYMOTION_CATEGORY), 'SOME CATEGORY')
        self.assertEquals(master.get(const.GNM_MASTERS_DAILYMOTION_TITLE), 'SOME DAILYMOTIONS TITLE')
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_REMOVE), datetime.date.today().strftime(const.VS_DATEFORMAT))
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_PREVENTMOBILEUPLOAD), 'prevent_mobile_upload')
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_CONTAINSADULTCONTENT), 'contains_adult_content')
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_OWNER), 'SOME OWNER')
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_PUBLISH), datetime.date.today().strftime(const.VS_DATEFORMAT))
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_TITLEID), 'SOME TITLEID')
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_INTENDEDUPLOADPLATFORMS), None)
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_UKONLY), 'ukonly')
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_HOLDINGIMAGE_16X9), None)
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_HOLDINGIMAGE_5X4), None)
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_UPLOADTYPE), None)
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_WHOLLYOWNED), 'wholly_owned')
        self.assertEquals(master.get(const.GNM_MASTERS_GENERIC_STATUS), 'Published')
        self.assertEquals(master.get(const.GNM_MASTERS_INTERACTIVE_TITLE), 'INTERACTIVE TITLE')
        self.assertEquals(master.get(const.GNM_MASTERS_INTERACTIVE_PROJECTREF), None)
        self.assertEquals(master.get(const.GNM_MASTERS_WEBSITE_STANDFIRST), 'SOME STANDFIRST')
        self.assertEquals(master.get(const.GNM_MASTERS_WEBSITE_BYLINE), 'AWESOME BYLINE')
        self.assertEquals(master.get(const.GNM_MASTERS_WEBSITE_TAGS), None)
        self.assertEquals(master.get(const.GNM_MASTERS_WEBSITE_HEADLINE), headline)
        self.assertEquals(master.get(const.GNM_MASTERS_WEBSITE_LINKTEXT), 'CLICK ME')
        self.assertEquals(master.get(const.GNM_MASTERS_WEBSITE_TRAIL), 'TRAIL OF THINGIES')
        self.assertEquals(master.get(const.GNM_MASTERS_MAINSTREAMSYNDICATION_TITLE), 'SYNDICATION TITLE')
        self.assertEquals(master.get(const.GNM_MASTERS_MAINSTREAMSYNDICATION_CATEGORY), 'SYNDICATION CATEGORY')
        self.assertEquals(master.get(const.GNM_MASTERS_MAINSTREAMSYNDICATION_KEYWORDS), None)
        self.assertEquals(master.get(const.GNM_MASTERS_MAINSTREAMSYNDICATION_DESCRIPTION), 'SYNDICATION DESCRIPTION')
        self.assertEquals(master.get(const.GNM_MASTERS_MAINSTREAMSYNDICATION_PUBLISH), datetime.date.today().strftime(const.VS_DATEFORMAT))
        self.assertEquals(master.get(const.GNM_MASTERS_MAINSTREAMSYNDICATION_CONTAINSADULTCONTENT), None)
        self.assertEquals(master.get(const.GNM_MASTERS_MAINSTREAMSYNDICATION_AUTHOR), 'SYNDICATION AUTHOR')
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_KEYWORDS), None)
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_ALLOWRESPONSES), 'allow_responses')
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_PUBLISH), datetime.date.today().strftime(const.VS_DATEFORMAT))
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_UGCPOLICY), 'SOME UGCPOLICY')
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_CONTAINSADULTCONTENT), 'contains_adult_content')
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_UPLOADSTATUS), 'Upload Failed')
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_ALLOWEMBEDDING), 'allow_embedding')
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_ALLOWRATINGS), 'allow_ratings')
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_DESCRIPTION), 'YOUTUBE DESCRIPTION')
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_TITLE), 'YOUTUBE TITLE')
        ...
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_COMMERCIALPOLICY), 'THERE WAS A POLICY')
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_MATCHINGPOLICY), 'matching_policy')
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_ALLOWCOMMENTS), 'allow_comments')
        self.assertEquals(master.get(const.GNM_MASTERS_YOUTUBE_CATEGORY), 'A CATEGORY')
    ...
    def test_title_id_script_fail_gracefully(self):
        ...
        commission.add_project(project)
        master = self._create_master()
        settings.TITLE_ID_SCRIPT = u'/this_path_is_highly_unlikely_to_exist/fake_name.pl'
        master.generate_and_set_new_title_id(project)
        title_id = master.get(const.GNM_MASTERS_GENERIC_TITLEID)
        self.assertEquals(title_id, '')  # Empty string is expected even if script fails
    ...
    def test_get_parent_project(self):
        ...
        self.assertEquals(master.get_parent_project().id, project.id)
        self.assertEquals(master2.get_parent_project().id, project.id)
