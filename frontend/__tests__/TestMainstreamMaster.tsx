  ...
  it("should populate data on load", (done) => {
    ...
    moxios.wait(() => {
      ...
      result.then(
        () => {
          rendered.update();
          expect(rendered.find("input#ms-title").props()["value"]).toEqual(
            "some title"
          );
          expect(
            rendered.find("textarea#ms-description").props()["value"]
          ...
          expect(
            rendered.find("input#ms-restricted").props()["checked"]
          ).toEqual(true);
          done();
        },
        (err) => done.fail(err)
