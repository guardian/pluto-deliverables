...
describe("VidispineJobProgress", ()=>{
    ...
    it("should callout to VS on mount to get information on the given job id and set up a timer", async ()=>{
        ...
        fs.readFile(__dirname + "/../sample-job.json", async (err, mockJobText) => {
            if (err) throw err;

            const rendered = mount(<VidispineJobProgress jobId="VX-1234"
    ...
    function readFilePromise(path:string):Promise<string> {
        return new Promise((resolve, reject)=>fs.readFile(path, (err, buffer)=>{
            if(err) reject(err);
