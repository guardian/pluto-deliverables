...
describe("SyndicationTrigger", ()=>{
    ...
    it("should display an enabled button if uploadStatus is NOT_UPLOADING, clicking it should activate send and call sendInitiated", (done)=>{
        ...
        const rendered = mount(<SyndicationTrigger uploadStatus={null}
                                                   platform="Test"
                                                   projectId={1234}
                                                   assetId={BigInt(5678)}
    ...
    it("should display an enabled button if uploadStatus is WAITING_FOR_START, clicking it should display logs", (done)=>{
        ...
        const rendered = mount(<SyndicationTrigger uploadStatus="Ready for Upload"
                                                   platform="Test"
                                                   projectId={1234}
                                                   assetId={BigInt(5678)}
                                                   sendInitiated={sendInitiatedCb}
        ...
        moxios.wait(async ()=>{
            try {
                ...
                await act(async ()=>req.respondWith({
                    status: 200,
                    response: {
                        status: "ok"
                    }
    ...
    it("should display an enabled button if uploadStatus is IN_PROGRESS, clicking it should display logs", (done)=>{
        ...
        const rendered = mount(<SyndicationTrigger uploadStatus="Uploading"
                                                   platform="Test"
                                                   projectId={1234}
                                                   assetId={BigInt(5678)}
                                                   sendInitiated={sendInitiatedCb}
    ...
    it("should display an enabled button if uploadStatus is FAILED, clicking it should activate send and call sendInitiated", (done)=>{
        ...
        const rendered = mount(<SyndicationTrigger uploadStatus="Upload Failed"
                                                   platform="Test"
                                                   projectId={1234}
                                                   assetId={BigInt(5678)}
                                                   sendInitiated={sendInitiatedCb}
    ...
    it("should display a disabled upload and enabled log button if uploadStatus is COMPLETE, clicking it should display logs", (done)=>{
        ...
        const rendered = mount(<SyndicationTrigger uploadStatus="Upload Complete"
                                                   platform="Test"
                                                   projectId={1234}
                                                   assetId={BigInt(5678)}
                                                   sendInitiated={sendInitiatedCb}
