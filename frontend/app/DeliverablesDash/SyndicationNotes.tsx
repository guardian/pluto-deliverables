...
const SyndicationNotes:React.FC<SyndicationNotesProps> = (props) => {
    ...
    useEffect(()=>{
        const loadNotes = async ()=>{
            ...
            setLoading(true);
            try {
                const response = await axios.get<SyndicationNoteResponse>(`/api/asset/${props.deliverableId}/notes?limit=${props.initialMax ?? 100}`);
                setNotes(response.data.results);
                setTotalNotes(response.data.count);
                setLoading(false);
            } catch(err) {
                setLoading(false);
                SystemNotification.open(SystemNotifcationKind.Error, `Could not load notes for ${props.deliverableId}`);
            }
        }
    ...
    const saveAndClose = async ()=>{
        try {
            await axios.post(`/api/asset/${props.deliverableId}/notes/new`,enteredNote,{headers: {"Content-Type": "text/plain"}});
            setEnteredNote("");
            setShowingEntry(false);
            setReloadCounter((prev)=>prev+1);
            SystemNotification.open(SystemNotifcationKind.Info, "Saved note");
        } catch(err) {
            SystemNotification.open(SystemNotifcationKind.Error, `Could not save your note: ${err}`);
        }
    }
    ...
    return <>
        <Grid container direction="row" justify="space-between" alignItems="center">
            <Grid item>
                <Typography className={classes.headNote}>{loading ? "Loading" : `${totalNotes} notes`}</Typography>
                <ShowAllSelector value={expanded} onChange={(newValue)=>setExpanded(newValue)}/>
            </Grid>
            <Grid item>
        ...
        <ul className={classes.basicList}>
            { notesToDisplay.map((note,idx)=>
                <li key={idx} className={classes.basicList}>
                    <Typography className={clsx(classes.contentBox, expanded ? undefined : classes.collapsed)}>{note.content}</Typography>
                    <Typography className={classes.attributionBox}>{note.username} {formatTime(note.timestamp)}</Typography>
                </li>
            )}
        </ul>
        {
            showingEntry ? <Dialog open={showingEntry} onClose={closeEntryNoSave}>
                <DialogTitle>Add note</DialogTitle>
                <DialogContent>
                    <DialogContentText>Please enter the text of your note in the box below and click the "Save" button.
                    It will then be recorded alongside your username and the time that the note was made.
                    </DialogContentText>
