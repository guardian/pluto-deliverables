...
const DeliverablesDashList:React.FC<DeliverablesDashListProps> = (props) => {
    const [matchingAssets, setMatchingAssets] = useState<DenormalisedDeliverable[]>([]);

    const classes = useStyles();

    useEffect(()=>{
        const loadData = async ()=>{
            const response = await axios.get(`/api/dash/assets?startDate=${formatISO(props.startDate)}&endDate=${formatISO(props.endDate)}`,
                {validateStatus: ()=>true});
            switch(response.status) {
                case 200:
                    ...
                    break;
                default:
                    SystemNotification.open(SystemNotifcationKind.Error, `Could not load data, server returned ${response.status}`);
            }
        };
