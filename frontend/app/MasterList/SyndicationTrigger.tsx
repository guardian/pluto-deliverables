...
import {AccessAlarmOutlined, BackupOutlined, CheckCircleOutline, Error} from "@material-ui/icons";
...
const SyndicationTriggerButton:React.FC<SyndicationButtonProps> = (props) => {
    return <Tooltip title="Send to syndication partner">
...
const SyndicationTriggerIcon:React.FC<SyndicationIconProps> = (props) => {
    switch(props.uploadStatus) {
        ...
        case WAITING_FOR_START:
            return <Tooltip title="Waiting for start...">
                <AccessAlarmOutlined/>
            </Tooltip>
        case IN_PROGRESS:
            return <Tooltip title="Output is ongoing...">
                <CircularProgress/>
            </Tooltip>;
        case FAILED:
            return <Tooltip title="Output failed">
                <Error style={{color: "red"}}/>
            </Tooltip>
        case COMPLETE:
            return <Tooltip title="Output success">
...
const SyndicationTrigger:React.FC<SyndicationTriggerProps> = (props) => {
    ...
    const triggerUpload = async ()=>{
        try {
            await axios.post(`/api/bundle/${props.projectId}/asset/${props.assetId}/${props.platform}/send`);
    ...
    return <Grid container direction="row" alignItems="center">
        <Grid item>{
            props.uploadStatus==IN_PROGRESS || props.uploadStatus==WAITING_FOR_START ?
                null :
