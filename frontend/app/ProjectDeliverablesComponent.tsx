...
const ProjectDeliverablesComponent:React.FC<RouteComponentProps> = ()=>{
    ...
    const loadRecord = async () => {
        ...
        try {
            const response = await axios.get(`/api/deliverables?project_id=${projectid}`);
            return Promise.all([
    ...
    return (
        <>
            ...
            <span>location: <a href="pluto:openfolder:fixme">/path/to/folder</a></span>
            </Typography>
            <hr/>
            <span className={classes.buttonContainer}>
                <Button className={classes.buttons} variant="outlined" onClick={()=>doRefresh()}>Refresh</Button>
                <Button className={classes.buttons} variant="outlined">Delete</Button>
            </span>
            <Paper elevation={3}>
                <TableContainer>
                    <Table className={classes.table}>
                        ...
                        <TableBody>
                            {
                                deliverables.map((del, idx)=><TableRow key={del.id.toString()}>
                                    <TableCell>
                                        <input type="checkbox" onChange={evt=>{
                                            console.log(`checkbox ${del.id} changed: ${evt.target.checked}`);
                                            if(evt.target.checked) {
                                                setSelectedIDs(prevContent=>prevContent.concat(del.id));
                                            } else {
                                                setSelectedIDs(prevContent=>prevContent.filter(value=>value!==del.id))
                                            }
                                        }}/>
                                    </TableCell>
